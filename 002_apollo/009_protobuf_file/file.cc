...
bool CopyFile(const std::string &from, const std::string &to) {
  ...
  if (!src) {
    std::cout << "Source path could not be normally opened: " << from;
...
bool RemoveAllFiles(const std::string &directory_path) {
  ...
  while ((file = readdir(directory)) != nullptr) {
    ...
    if (unlink(file_path.c_str()) < 0) {
      std::cout << "Fail to remove file " << file_path << ": " << strerror(errno);
      closedir(directory);
      return false;
    }
  }
...
bool GetType(const string &filename, FileType *type) {
  ...
  if (S_ISDIR(stat_buf.st_mode) != 0) {
    ...
    *type = TYPE_FILE;
    ...
    std::cout << "failed to get type: " << filename;
    return false;
  }
  return true;
}
...
bool DeleteFile(const string &filename) {
  ...
  if (type == TYPE_FILE) {
    if (remove(filename.c_str()) != 0) {
      std::cout << "failed to remove file: " << filename;
      return false;
    }
    return true;
  }
  ...
  while ((dir_info = readdir(dir)) != nullptr) {
    ...
    if (!GetType(temp_file, &temp_type)) {
      std::cout << "failed to get file type: " << temp_file;
      closedir(dir);
      return false;
    }
    if (type == TYPE_DIR) {
